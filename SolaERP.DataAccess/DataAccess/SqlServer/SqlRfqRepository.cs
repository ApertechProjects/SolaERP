...
namespace SolaERP.DataAccess.DataAccess.SqlServer
{
    public class SqlRfqRepository : IRfqRepository
    {
        ...
        public async Task<List<RfqVendors>> GetRfqVendors(int rfqMainId)
        {
            await using var command = _unitOfWork.CreateCommand() as DbCommand;
            command.CommandText =
                @"select CAST(ROW_NUMBER() OVER(ORDER BY RFQVendorResponseId ASC) AS INT) AS RowNumber, vr.VendorCode,v.VendorName, vr.Date 
        ...
        public async Task<List<RFQDeadlineFinishedMailForBuyerDto>> GetRFQDeadlineFinished()
        {
            ...
            command.CommandText = @$"select 
    										...
    										rfqm.BusinessUnitId,
											bu.BusinessUnitName
									from Procurement.RFQMain as rfqm
									join Config.BusinessUnits as bu on bu.BusinessUnitId=rfqm.BusinessUnitId
									where rfqm.Status=1 and rfqm.RFQDeadline<getDate()";
        ...
        public async Task<List<RFQUserData>> GetRFQVendorUsersMailIsSentDeadLineFalse()
        {
            ...
            command.CommandText = @$"
				...
				INNER JOIN Config.BusinessUnits as bu on bu.BusinessUnitId=rm.BusinessUnitId
            ...
            await using DbDataReader reader = await command.ExecuteReaderAsync();
            List<RFQUserData> datas = new();

            while (await reader.ReadAsync())
            {
                datas.Add(new RFQUserData()
                {
        ...
        public async Task UpdateRFQSendManualBiddingType(int rfqMainId)
        {
            ...
            using (var command = _unitOfWork.CreateCommand() as DbCommand)
            {
                ...
                command.Parameters.AddWithValue(command, "@currentDate", bakuDateTime.ToString("yyyy-MM-dd"));
                command.Parameters.AddWithValue(command, "@rfqMainId", rfqMainId);
