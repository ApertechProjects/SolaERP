...
namespace SolaERP.DataAccess.DataAccess.SqlServer
{
	public class SqlSupplierEvaluationRepository : ISupplierEvaluationRepository
	{
		...
		public async Task<bool> ModifyVendorBusinessCategoryActivityAsync(int vendorBusinessCategoryId, bool active)
		{
			try
			{
				using (var command = _unitOfWork.CreateCommand() as DbCommand)
				{
					command.CommandText = @"SET NOCOUNT OFF EXEC SP_VendorBusinessCategory_U @VendorBusinessCategoryId,
                                                                          @Active";


					command.Parameters.AddWithValue(command, "@VendorBusinessCategoryId", vendorBusinessCategoryId);
					command.Parameters.AddWithValue(command, "@Active", active);
