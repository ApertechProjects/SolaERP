...
";
        ...
        public async Task<bool> SendUsingTemplate<T>(string subject, T viewModel, string templateName, string imageName,
            List<string> tos)
        {
            if (tos.Count == 0)
                return false;
        ...
        public async Task SendQueueUsingTemplate<T>(string subject, T viewModel, string templateName, string imageName,
            List<string> tos)
        {
            var fileRootPath = Path.GetFullPath(@"wwwroot/sources/templates");
            var imageRootPath = Path.GetFullPath(@"wwwroot/sources/images");
        ...
        public async Task SendMailForRequest(HttpResponse response, List<EmailTemplateData> templates,
            List<UserList> users, EmailTemplateKey key, int? sequence, string businessUnitName,
            string rejectReason = "")
        {
            for (int i = 0; i < users.Count; i++)
            {
                ...
                {
                    await SendUsingTemplate(requestPending.Subject, requestPending, requestPending.TemplateName(),
                        null, new List<string> { to });
                });
            }
        ...
        public async Task SendRequestMailsForChangeStatus(HttpResponse response, List<UserList> users, int? sequence,
            string businessUnitName, string rejectReason)
        {
            ...
            if (userREQP.Count > 0)
            {
                ...
                await SendMailForRequest(response, templates, userREQP, EmailTemplateKey.REQP, sequence,
                    businessUnitName);
            }
            ...
            if (userREQA.Count > 0)
            {
                ...
                await SendMailForRequest(response, templates, userREQA, EmailTemplateKey.REQA, sequence,
                    businessUnitName);
            }
            ...
            if (userREQR.Count > 0)
            {
                ...
                await SendMailForRequest(response, templates, userREQR, EmailTemplateKey.REQR, sequence,
                    businessUnitName, rejectReason);
            }
            ...
            if (userREQH.Count > 0)
            {
                ...
                await SendMailForRequest(response, templates, userREQH, EmailTemplateKey.REQH, sequence,
                    businessUnitName);
            }
        }
        ...
        public async Task CheckLastApproveStageAndSendMailToVendor(int vendorId, int sequence, int approveStatus,
        ...
        public async Task CheckLastApproveAndSendMailToUser(int userId, int sequence, int approveStatus,
            HttpResponse response)
        {
            var stageCount = await _approveStageService.GetStageCountAsync(Procedures.Vendors);
            if (stageCount == sequence && approveStatus == 1)
        ...
        public async Task SendMailToAdminstrationForApproveRegistrationForAutoApprove(int userId,
            ...
            foreach (IGrouping<string, EmailPersonDto> group in grouped)
            {
                ...
                if (adminEmails.Count > 0)
                {
                    ...
                    VM_RegistrationIsPendingAdminApprove adminApprove =
                        new((UserRegisterType)user.UserTypeId, changedFields)
                        {
                            Body = new HtmlString(templateData.Body),
                            CompanyName = companyName,
                            Header = templateData.Header,
        ...
        public async Task SendSupportMail(int userId, string subject, string body,
            List<AttachmentSaveModel> attachments)
        {
            List<Task> emails = new List<Task>();
            ...
            for (int i = 0; i < attachments.Count; i++)
            {
                ...
                var attachmentDownloadLink =
                    _fileUploadService.GetDownloadFileLink(attachment.FileLink, Modules.Support);
            ...
            VM_Support supportEmailVM =
                new VM_Support(user.Language.ToString(), subject, body, user.Email, attachmentsText)
                {
                    Language = (Language)Enum.Parse(typeof(Language), user.Language.ToString())
                };
        ...
        public async Task SendRFQVendorApproveMail(List<RfqVendorToSend> users)
        {
            ...
            {
                VM_RFQVendorApprove emailVM =
                    new VM_RFQVendorApprove(user.Language, user.Email, user.VendorName, 1, user.RFQDeadline, user.RFQNo)
                    {
        ...
        public async Task SendRFQDeadlineFinishedMailForBuyer(List<RFQDeadlineFinishedMailForBuyerDto> datas)
        {
            try
            {
                ...
                {
                    VM_RFQDeadlineFinishedMailForBuyer emailVM =
                        new VM_RFQDeadlineFinishedMailForBuyer("en", data.BuyerName, data.RFQNo, data.RFQDeadline,
                            data.RFQMainId, data.BusinessUnitName)
                        {
                            Language = (Language)Enum.Parse(typeof(Language), "en")
                        };
