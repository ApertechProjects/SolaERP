...
namespace SolaERP.Persistence.Services
{
	public class VendorService : IVendorService
	{
			...
			for (int i = 0; i < taxModel.VendorIds.Count; i++)
			{
				...
				if (result)
				{
					...
					if (taxModel.Status == 2)
						...
						await _mailService.SendMailToAdminstrationForApproveRegistration(userId, null);
		...
		public async Task<ApiResponse<VM_VendorCard>> GetAsync(int vendorId)
		{
			...
			var vendorBuCategoriesForActive = await _supplierRepository.GetVendorBuCategoriesAsync(header.VendorId);
			var activeMatchedBuCategories = vendorBuCategoriesForActive.Where(x => x.Active == true).ToList();
			...
			VM_VendorCard vendorModel = new VM_VendorCard()
			{
				...
				ActiveItemCategories = activeMatchedBuCategories,
			};
