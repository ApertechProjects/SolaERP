    ...
    public class RequestService : IRequestService
    {
            ...
            int rejectReasonId, string businessUnitName, int? sequence, HttpResponse response)
        {
            ...
            if (result)
            {
                ...
                if (sequence == stageCount && approveStatus == 1)
                {
                    ...
                    await _mailService.SendMailForRequest(response, templates, requestEmailSendUser,
                        EmailTemplateKey.REQA, sequence, businessUnitName);
                }
                else if (approveStatus == 2)
                {
                    ...
                    await _mailService.SendMailForRequest(response, templates, requestEmailSendUser,
                        EmailTemplateKey.REQR, sequence, businessUnitName);
                }
            }
        ...
        public async Task<bool> SendToApproveAsync(string name, List<int> requestMainIds, HttpResponse response)
        {
            ...
            var sendToApproveTasks = requestMainIds.Select(async requestId =>
            {
                ...
                {
                    await _mailService.SendMailForRequest(response, templates, requestEmailSendUser,
                        EmailTemplateKey.REQP, 1, businessUnitName);
                });
            }).ToList();
        ...
        public async Task<ApiResponse<bool>> UpdateBuyerAsync(RequestSetBuyer requestSetBuyer, string name)
        {
            ...
            var details = requestSetBuyer.RequestDetails;

            string buyerEmail =
                await _buyerService.FindBuyerEmailByBuyerName(requestSetBuyer.Buyer, requestSetBuyer.BusinessUnitId);

            string businessUnitName =
                await _buyerService.FindBusinessUnitNameByBuyerName(requestSetBuyer.Buyer,
                    requestSetBuyer.BusinessUnitId);
            ...
            {
                int requestMainId = await _requestMainRepository.GetRequestMainIdByRequestDetailId(
                    details[0].RequestDetailId);
                if (requestMainId != null) {
                    buyerData.RequestMainId = requestMainId;
                }
            }


            _taskQueue.QueueBackgroundWorkItem(async token => { await _mailService.SendRequestBuyerMail(buyerData); });
