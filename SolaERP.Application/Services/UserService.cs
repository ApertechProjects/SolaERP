...
namespace SolaERP.Persistence.Services
{
	public class UserService : IUserService
	{
		...
		public async Task<ApiResponse<int>> UserRegisterAsync(UserRegisterModel model, HttpResponse response)
		{
		...
		public async Task<ApiResponse<bool>> UserChangeStatusAsync(string name, UserChangeStatusModel model, HttpResponse response)
		{
			var userId = await _userRepository.ConvertIdentity(name);
			if (model.Id == 0)
			...
			await _mailService.CheckLastApproveAndSendMailToUser(model.Id, model.Sequence, model.ApproveStatus, response);
		...
		public async Task<ApiResponse<bool>> UserChangeStatusAsync(string name, List<UserChangeStatusModel> model, HttpResponse response)
		{
			...
			foreach (var item in model)
			{
				...
				await _mailService.CheckLastApproveAndSendMailToUser(item.Id, response);
			}
